...
export function PublicEndpoint() {
	return SetMetadata<string, string[]>(AUTH_META_KEY, [PUBLIC_ENDPOINT_DENOTATOR]);
}
...
export class AuthGuard implements CanActivate {
	...
	public async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		if (allMetadata.some((m) => m === PUBLIC_ENDPOINT_DENOTATOR)) {
			...
			request.headers.authorization.startsWith(TOKEN_PREFIX)
		) {
			try {
				return !!(await this.jwtAuthService.validateToken(
					request.headers.authorization.replace(TOKEN_PREFIX, '')
				));
			} catch (e) {
